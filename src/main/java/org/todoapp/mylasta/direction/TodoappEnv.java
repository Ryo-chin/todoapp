...
public interface TodoappEnv {
    ...
    /** The key of the configuration. e.g. /tmp/lastaflute/todoapp */
    ...
    /** The key of the configuration. e.g. returnpath@todoapp.org */
    String MAIL_RETURN_PATH = "mail.return.path";

    /** The key of the configuration. e.g. todoapp-support@annie.example.com */
    String MAIL_ADDRESS_SUPPORT = "mail.address.support";

    /** The key of the configuration. e.g. org.h2.Driver */
    String JDBC_DRIVER = "jdbc.driver";

    /** The key of the configuration. e.g. jdbc:h2:file:$classes(org.todoapp.dbflute.allcommon.DBCurrent.class)/../../etc/testdb/todoappdb */
     ...
     * The value is, e.g. /tmp/lastaflute/todoapp <br>
     ...
     * The value is, e.g. returnpath@todoapp.org <br>
     ...
     * The value is, e.g. todoapp-support@annie.example.com <br>
     * comment: Mail Address for Todoapp Support
     * @return The value of found property. (NotNull: if not found, exception but basically no way)
     */
     ...
     * The value is, e.g. jdbc:h2:file:$classes(org.todoapp.dbflute.allcommon.DBCurrent.class)/../../etc/testdb/todoappdb <br>
     ...
     * The value is, e.g. localhost:8090/todoapp <br>
     ...
     * @author FreeGen
     */
    public static class SimpleImpl extends ObjectiveConfig implements TodoappEnv {

        /** The serial version UID for object serialization. (Default) */
        private static final long serialVersionUID = 1L;

        public String getLastaDiSmartDeployMode() {
            return get(TodoappEnv.lasta_di_SMART_DEPLOY_MODE);
        }

        public String getDevelopmentHere() {
            return get(TodoappEnv.DEVELOPMENT_HERE);
        }

        public boolean isDevelopmentHere() {
            return is(TodoappEnv.DEVELOPMENT_HERE);
        }

        public String getEnvironmentTitle() {
            return get(TodoappEnv.ENVIRONMENT_TITLE);
        }

        public String getFrameworkDebug() {
            return get(TodoappEnv.FRAMEWORK_DEBUG);
        }

        public boolean isFrameworkDebug() {
            return is(TodoappEnv.FRAMEWORK_DEBUG);
        }

        public String getTimeAdjustTimeMillis() {
            return get(TodoappEnv.TIME_ADJUST_TIME_MILLIS);
        }

        public Long getTimeAdjustTimeMillisAsLong() {
            return getAsLong(TodoappEnv.TIME_ADJUST_TIME_MILLIS);
        }

        public String getLogLevel() {
            return get(TodoappEnv.LOG_LEVEL);
        }

        public String getLogConsoleLevel() {
            return get(TodoappEnv.LOG_CONSOLE_LEVEL);
        }

        public String getLogFileBasedir() {
            return get(TodoappEnv.LOG_FILE_BASEDIR);
        }

        public String getMailSendMock() {
            return get(TodoappEnv.MAIL_SEND_MOCK);
        }

        public boolean isMailSendMock() {
            return is(TodoappEnv.MAIL_SEND_MOCK);
        }

        public String getMailSmtpServerMainHostAndPort() {
            return get(TodoappEnv.MAIL_SMTP_SERVER_MAIN_HOST_AND_PORT);
        }

        public String getMailSubjectTestPrefix() {
            return get(TodoappEnv.MAIL_SUBJECT_TEST_PREFIX);
        }

        public String getMailReturnPath() {
            return get(TodoappEnv.MAIL_RETURN_PATH);
        }

        public String getMailAddressSupport() {
            return get(TodoappEnv.MAIL_ADDRESS_SUPPORT);
        }

        public String getJdbcDriver() {
            return get(TodoappEnv.JDBC_DRIVER);
        }

        public String getJdbcUrl() {
            return get(TodoappEnv.JDBC_URL);
        }

        public String getJdbcUser() {
            return get(TodoappEnv.JDBC_USER);
        }
        ...
        public Integer getJdbcConnectionPoolingSizeAsInteger() {
            return getAsInteger(TodoappEnv.JDBC_CONNECTION_POOLING_SIZE);
        }
